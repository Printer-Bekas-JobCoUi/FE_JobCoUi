...
export default function Login() {
  ...
  const [email, setEmail] = useState('admin@jobco.id') // Updated default email
  ...
  return (
    <div className="min-h-screen w-full flex bg-white relative overflow-hidden">
      ...
      <div
        className={`flex flex-col justify-center items-center p-8 sm:p-12 lg:p-16 relative z-20 bg-white transition-all duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] ${isSuccess ? 'w-full translate-x-0' : 'w-full lg:w-1/2'
          }`}
      >
        <div className={`w-full max-w-md space-y-8 transition-opacity duration-300 ${isSuccess ? 'opacity-0' : 'opacity-100'}`}>
          ...
          <div className="space-y-6">
            {error && (
              <div className="bg-rose-50 border border-rose-100 text-rose-600 px-4 py-3 rounded-xl flex items-center gap-3 animate-shake">
            ...
            <div className="pt-4">
              <button
                className={`w-full flex items-center justify-center gap-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-orange-500/20 active:scale-[0.98] transition-all duration-300 group ${(isLoading || isSuccess) ? 'opacity-80 cursor-wait' : ''
                  }`}
                type="button"
                onClick={handleLogin}
                disabled={isLoading || isSuccess}
      ...
      <div
        className={`hidden lg:flex relative bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-950 overflow-hidden items-center justify-center p-12 transition-all duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] ${isSuccess ? 'w-0 opacity-0' : 'flex-1 w-1/2'
          }`}
      >
